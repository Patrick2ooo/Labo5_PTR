...
void video_task(void *cookie)
{
	...
	uint8_t *convolution_buffer = (uint8_t *)malloc(WIDTH * HEIGHT*sizeof(uint8_t) );
	uint8_t *greyscale_buffer = (uint8_t *)malloc(WIDTH * HEIGHT*sizeof(uint8_t));
	...
	while (priv->ctl->running) {
		...
		fseek(priv->file,
		     indexFrame * WIDTH * HEIGHT * BYTES_PER_PIXEL,
		      SEEK_SET);
		...
		for (; indexFrame < NB_FRAMES; indexFrame++) {
			...
			if(priv->ctl->greyscale_running && !priv->ctl->convolution_running){
				
				rgba_to_grayscale32(&img, &greyscale_img); 
				memcpy(get_video_buffer(), priv->greyscale_buffer, WIDTH * HEIGHT * BYTES_PER_PIXEL);
			} else if(priv->ctl->convolution_running && !priv->ctl->greyscale_running){
				
				rgba_to_grayscale8(&img, greyscale_buffer); 
				
                convolution_grayscale(greyscale_buffer, convolution_buffer , WIDTH, HEIGHT ); 
				
                grayscale_to_rgba(convolution_buffer, &rgba_result); 

                memcpy(get_video_buffer(), rgba_result.data, WIDTH * HEIGHT * BYTES_PER_PIXEL);
			}
			else{
